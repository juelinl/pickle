...
template<class T>
inline int get_num_matched(int top_k, const GroundTruthPtr &truth, const std::vector<T> &results) {
    size_t num_queries = results.size();
    int total_matched{0};
    for (int i = 0; i < num_queries; i++) {
        const auto& search_result = results.at(i);
...
int main(int argc, char *argv[]) {
    ...
    HNSWGraph graph(config.M, config.ef_construction, config.df);
    timer.start();
    auto ext_ids =
    ...
    timer.start();
    graph.Build<uint8_t, 128>(ext_ids, feat);

//    ATEN_DTYPE_SWITCH(feat->_data_type, DType, {
////        graph.Build<DType, std::dynamic_extent>(ext_ids, feat);
//        ATEN_DIM_SWITCH(dim, Dim, {
//                graph.Build<DType, Dim>(ext_ids, feat);
//        });
//    });
    timer.end();
...
//        timer.start();
//        std::vector<external_id_t> ext_ids =
//                GetRandIndices<external_id_t>(config.max_elements);
//        timer.end();
//        std::cout << "Get random indices in " << timer.seconds() << "secs" << std::endl;
//
//        timer.start();
//        graph.Build(ext_ids, feat);
//        timer.end();
//
//        std::cout << "Graph construction in " << timer.seconds() << " secs" << std::endl;
////        if (config.index_path != "") {
////            timer.start();
////            Serializer::to_disk(config.index_path, graph);
////            timer.end();
...
//    }
    ...
    if (config.query_path != "" && config.truth_path != "") {
        ...
        for (auto top_k : top_k_vec) {
            for (auto search_ef: ef_search_vec) {
                std::vector<EntryVector> results(num_queries);
                timer.start();
                ATEN_DTYPE_SWITCH(query->_data_type, DType, {
                    for (int i = 0; i < num_queries; i++) {
                        ...
                        results.at(i) = graph.Search<DType, Dim>(top_k, search_ef, q_data);
                    }
                });
                timer.end();
                ...
                std::cout << "top_k=" << top_k << " search_ef=" << search_ef << " recall=" << recall << " qps=" << qps << std::endl;
            }
        }
    }
}