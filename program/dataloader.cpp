...
Array2DPtr LoadArray2D(const std::string &filename, size_t max_elements,
  ...
  if (to_float) {
    ...
    if (extension == "fbin") {
      size_t num_bytes = ret->_word_size * ret->_shape[0] * ret->_shape[1];
...
template <> std::vector<uint32_t> getRandomIndices<uint32_t>(size_t);
template <> std::vector<int32_t> getRandomIndices<int32_t>(size_t);
template <> std::vector<uint64_t> getRandomIndices<uint64_t>(size_t);
template <> std::vector<int64_t> getRandomIndices<int64_t>(size_t);
} // namespace pickle