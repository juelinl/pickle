...
namespace pickle::v1 {
    ...
    public:
        ...
        template<class T, size_t Dim>
        inline std::vector<Entry> SelectHeuristic(int top_k, int level, MaxHeap& candidates, bool keep_pruned) {
            MinHeap W;
            MinHeap Wd;
            std::vector<Entry> R;
            ...
            if (keep_pruned) {
                ...
                while(other.size() + R.size() < top_k && !Wd.empty()) {
    ...
    void HNSWGraph::Insert(external_id_t ext_id, std::span<const T, Dim> query) {
        ...
        std::vector<std::vector<Entry>> entry_list(top_search_level + 1);
...
//                auto entries = SelectHeuristic<T, Dim>(top_k, level, ret, true);
        ...
        for (int level = 0; level <= l; level++) {
            ...
            if (level <= top_inserted_level) {
                ...
                AddNode<T, Dim>(vid, neighbors, query, level);
